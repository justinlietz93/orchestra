...
from .domain import Agent, WorkflowState
...
class StateStore:
    ...
    def load(self) -> WorkflowState:
        ...
        if "auditor_revision" not in value:
            state = self._infer_legacy_auditor_revision(state)
        return state
    ...
    def _infer_legacy_auditor_revision(
        self, state: WorkflowState
    ) -> WorkflowState:
        if state.active_agent != Agent.AUDITOR or not state.last_manifest:
            return state
        try:
            manifest = self.project_root / state.last_manifest
            with manifest.open("r", encoding="utf-8") as handle:
                event = json.load(handle)
        except (OSError, ValueError, json.JSONDecodeError):
            return state
        is_revision = (
            event.get("source_agent") == Agent.GUARDIAN.value
            and event.get("guardian_subject") == Agent.AUDITOR.value
            and event.get("result") == "Fail"
        )
        return replace(state, auditor_revision=is_revision)
