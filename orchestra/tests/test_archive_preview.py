...
class ArchivePreviewTests(unittest.TestCase):
    ...
    def test_auditor_revision_is_locked_to_current_version(self) -> None:
        previews = build_previews(
            self.root,
            self.index,
            self.current,
        ...
        self.assertIn(
            "RECORD IN CURRENT VERSION", previews[ArchiveAction.CONTINUE].text
        )
        self.assertTrue(
            previews[ArchiveAction.CONTINUE].text.endswith("p5-b3-v10")
        )
        self.assertFalse(previews[ArchiveAction.NEW_BRANCH].enabled)
        self.assertFalse(previews[ArchiveAction.NEW_PHASE].enabled)
